...
User defined processes (UDPs) are one out of two standardised options that APEx offers to publish algorithms as a service.
...
are made to increase uniformity across the [APEx Algorithm Catalogue](https://algorithm-catalogue.apex.esa.int/).
...
Keep in mind that APEx offers an [Algorithm Porting](../propagation/porting.md) service to help you with these steps
if needed.
...
to consider them to avoid that users of the algorithm catalogue would be confused by variations in process parameter names.
...
In the standard openEO processes `load_collection` and `load_stac`, spatial filtering done through the argument called `spatial_extent`, which support multiple types: a bounding box object, an inline GeoJSON object, a vector cube, or it can even be left empty (`null`). 
...
spatial filtering in these processes, and also use `spatial_extent` as the parameter name unless there are good reasons otherwise.
...
The Python client has a convenience function called [`Parameter.spatial_extent()`](https://open-eo.github.io/openeo-python-client/api.html#openeo.api.process.Parameter.spatial_extent) to create this parameter when building your UDP via Python.
...
A [template](https://github.com/ESA-APEx/apex_documentation/blob/main/guides/udp_description_template.md) is available to help you structure your documentation.
...
Next, you will need to upload a generic JSON to the [APEx Algorithm Catalogue](https://algorithm-catalogue.apex.esa.int/) to register your process. For now, this is
done by creating a pull request in the [APEx Algorithm Catalogue GitHub repository](https://github.com/ESA-APEx/apex_algorithms).
