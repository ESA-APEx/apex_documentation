...
One of the primary goals of APEx is to facilitate the transition of Earth Observation R&D results into re-usable, scalable, cloud-based on-demand services.
...
workflows and algorithms, or conventional (open-source) desktop software solutions. Maintaining these results beyond the lifetime of a specific project is a challenge and this is a key objective for APEx. 

The vision for the ESA APEx initiative is adopt FAIR data principles and ensure tht project results remain Findable and Accessible and that they become Interoperable and Reusable. For value added products, this mainly involves ingestion in an ESA controlled persistent storage solution and exposing these products accordingly (using STAC and other technologies). 

Maintaining workflows and algorithms in a FAIR compliant way requires adoption of cloud service concepts and implementations. Therefore, the APEx envisioned solution for workflows and algorithms is to make them available in a public cloud (platform) in co-location with the key EO data archives as on-demand services using standardized web service APIs. These web services can then be triggered by users with their user specific parametrisation to produce results for an area and time frame of interest

However, developing on-demand services can be a challenging task for many projects and depends a lot on the complexity of an workflow/algorithm and its initial implementation.
APEx intends to facility the transition to cloud based services by providing guidelines, best practices and tool that support EO R&D projects in overcoming these challenges.
...
* Onboarding and integration of your service into APEx catalogue
...
After your UDP is created, it can be registered in the APEx Algorithm Catalogue and associated with one or more backends or platforms capable of running it.
...
If your platform meets the APEx [interoperability and compliance guidelines](../interoperability/algohostingenv.md), and you have onboarded your platfrom to the ESA Network of Resources (or plan to do that), 
...
The level of support depends on if you are:

* An ongoing or upcoming ESA EOP project
* A completed EOP project
* A non ESA project
* Any other EO application or algorithm developer
