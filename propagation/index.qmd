...
### Choosing between openEO and OGC Application Packages

To assist projects in selecting the right approach, we’ve created dedicated pages that explain both [Application Package via OGC API Processes](./ogc_processes.md)
and [openEO](./openeo_services.md) based services. These resources provide insights into the differences, advantages, and examples for each option, along with getting-started instructions. This guidance helps project teams understand each framework’s unique strengths and get started quickly with the one that best fits their objectives.
