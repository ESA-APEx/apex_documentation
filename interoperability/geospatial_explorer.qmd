...
A string that is required to render and conrol time series layers. It should have a value of either `Days`, `Months`, `Years`. This will determine the behaviour of the UI for layer groups
...
* `timestamps`: An optional array of Unix timestamps or ISO8601 date strings. This allows the configuration of time series visualisation within the layergroup. If multiple timestamps exist within the layergroup and the timeframe property is set a datepicker/stepper control will be rendered to cycle which layer is visible.
