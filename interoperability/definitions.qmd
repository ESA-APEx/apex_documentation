...
flowchart TD
    ...
    OPENEO_OGC --> | used by | LAST_MILE
    LAST_MILE --> | via | HOSTING_PLATFORM
```
...
value chain, and the web service API's that offer EO derived information. So this could for instance be the integration
of EO derived parcel statistics into the field management software of a farmer. In the ESA context, 'Green Transition Information
Factories' are also examples of last mile applications.

Typically, each user group and domain will have its own set of purpose-built tools. The IT integrator or vendor building these tools will retrieve EO algorithm results via 
web service API's based on open standards. These results can then be further processed as needed, depending no the application.
Note that these applications are not necessarily EO-centric or even geospatial, but could be using an EO algorithm as a small part
or in the background.

The APEx algorithm catalog acts as a discovery tool for last mile application builders. It shows various technical parameters, 
the cost and potential limitations for specific use cases. This speeds up the discovery and selection process. Application builders
do need an account on the hosting platforms if they want to test or use the API. APEx is also not involved in further interactions 
between the last mile application and the platform.
