...
The _type_ field of these links should be set to _application/json_. Please provide a descriptive _title_ for each link,
...
APEx requires at least one check, to be able to correctly mark process that are (temporarily) unavailable. When this happens
the 'principal investigator' is informed, allowing to take action as desired. 
